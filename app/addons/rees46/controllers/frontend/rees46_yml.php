...
function check_xml ($string_xml) {
    $string_xml = preg_replace( "/\r|\n|\t/", '', $string_xml);
    $string_xml = preg_replace("/\s+/", ' ', $string_xml);
    $string_xml = trim($string_xml);
    return '<![CDATA['.$string_xml.']]>';
}
...
function fn_yml_get_rees46_yml($filename){
    ...
    foreach ($products as $product) {
        ...
        $query = "SELECT price FROM ?:product_prices WHERE product_id=".$product['id']." AND usergroup_id=0";
